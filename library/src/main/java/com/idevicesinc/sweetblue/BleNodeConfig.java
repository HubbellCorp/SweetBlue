...
public class BleNodeConfig
{
	 ...
	 * {@link BleDevice} is {@link BleDeviceState#BLE_CONNECTED}, calling {@link BleDevice#disconnect()} then {@link BleDevice#connect()}
	 ...
	 * If you are using {@link BleDevice#negotiateMtu(int)}, this may make things unstable.
	 */
	...
	@com.idevicesinc.sweetblue.annotations.Advanced
	@Nullable(Nullable.Prevalence.NORMAL)
	public HistoricalDataFactory historicalDataFactory						= HistoricalData::new;
	...
	public static interface HistoricalDataLogFilter
	{
		...
		public static enum Source implements UsesCustomNull
		{
			...
			/**
			 * Originates from {@link BleDevice#enableNotify(BleNotify)}.
			 *
			 * @see com.idevicesinc.sweetblue.NotificationListener.Type#NOTIFICATION
			 */
			NOTIFICATION,

			/**
			 * Originates from {@link com.idevicesinc.sweetblue.BleDevice#enableNotify(BleNotify)}.
			 *
			 * @see com.idevicesinc.sweetblue.NotificationListener.Type#INDICATION
			 */
			INDICATION,
		...
		public static class HistoricalDataLogEvent extends Event
		{
			...
			@Override public String toString()
			{
				...
				(
					...
					"charUuid", P_Bridge_Internal.charName(m_node.getIBleNode().getIManager(), charUuid()),
					"source", source(),
					"data", data()
				);
			}
	...
	public static double getTimeout(final TaskTimeoutRequestFilter.TaskTimeoutRequestEvent event)
	{
		...
		{
			filter_specific = device_nullable.getIBleDevice().conf_device().taskTimeoutRequestFilter;
		}
		else if( server_nullable != null )
		{
			filter_specific = server_nullable.getIBleServer().conf_node().taskTimeoutRequestFilter;
		}
		else
		{
		...
		final TaskTimeoutRequestFilter filter_mngr = manager.getConfigClone().taskTimeoutRequestFilter;
		...
		final Interval timeout = please != null ? please.interval() : Interval.DISABLED;
		final double toReturn = timeout != null ? timeout.secs() : Interval.DISABLED.secs();

		P_Bridge_Internal.checkPlease(event.manager().getIBleManager(), please, TaskTimeoutRequestFilter.Please.class);
