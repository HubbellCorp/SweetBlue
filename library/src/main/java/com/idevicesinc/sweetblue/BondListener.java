...
public interface BondListener extends GenericListener_Void<BondListener.BondEvent>
{
     ...
     */
    @Immutable
    class BondEvent extends Event implements UsesCustomNull
    {
        ...
        /**
         * Used to indicate whether this {@link BondEvent} is for bonding, or unbonding.
         */
        ...
        BondEvent(BleDevice device, Type type, Status status, int failReason, State.ChangeIntent intent)
        {
            m_device = device;
            m_status = status;
            m_type = type;
        ...
        {
            return new BondEvent(device, Type.NULL, Status.NULL, BleStatuses.BOND_FAIL_REASON_NOT_APPLICABLE, State.ChangeIntent.NULL);
        }
            ...
            else
            {
                return Utils_String.toString
                        (
                                ...
                                "type", type(),
                                "status", status(),
                                "failReason", CodeHelper.gattUnbondReason(failReason(), true),
                                "intent", intent()
                        );
            }
        }
