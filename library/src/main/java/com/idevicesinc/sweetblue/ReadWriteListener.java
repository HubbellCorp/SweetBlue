...
public interface ReadWriteListener extends GenericListener_Void<ReadWriteListener.ReadWriteEvent>
{
    ...
    public static enum Status implements UsesCustomNull
    {
         ...
         * etc. was called on {@link BleDevice#NULL}.
         */
        NULL_DEVICE,
        ...
        /**
         * The android device's chipset doesn't support an API in the native stack. For example, if you try to use {@link BleDevice#setPhyOptions(Phy, ReadWriteListener)},
         * and the device doesn't have a Bluetooth 5 radio.
         */
        ...
        /**
         * The {@link BleTransaction} provided was either not started or not the current.
         */
        INVALID_TRANSACTION;
    ...
    public static enum Type implements UsesCustomNull
    {
         ...
         * or {@link BleDevice#enableNotify(UUID, Interval, ReadWriteListener)} where a force-read timeout is invoked. You should never see this
         * {@link Type} via {@link #onEvent(Event)}. Any event with this type will be emitted in the {@link NotificationListener}.
         */
        PSUEDO_NOTIFICATION,
        ...
        /**
         * Returns <code>true</code> for every {@link ReadWriteListener.Type} except {@link #isWrite()}. Overall this convenience method is meant
         * to tell you when we've <i>received</i> something from the device as opposed to writing something to it.
         */
    ...
    public static enum Target implements UsesCustomNull
    {
        ...
        /**
         * The {@link ReadWriteListener.ReadWriteEvent} is coming in from using {@link BleDevice#negotiateMtu(int, ReadWriteListener)} or overloads.
         */
        MTU,
        ...
        /**
         * The {@link ReadWriteEvent} is coming in from using {@link BleDevice#setPhyOptions(Phy, ReadWriteListener)} to set Bluetooth 5 specific features.
         */
        PHYSICAL_LAYER;
    ...
    public static class ReadWriteEvent extends com.idevicesinc.sweetblue.utils.Event implements com.idevicesinc.sweetblue.utils.UsesCustomNull
    {
        ...
        ReadWriteEvent(BleDevice device, BleOp bleOp, ReadWriteListener.Type type, ReadWriteListener.Target target, ReadWriteListener.Status status, int gattStatus, double totalTime, double transitTime, boolean solicited)
        {
            ...
            final byte[] bytes = bleOp.getData().getData();
            this.m_data = bytes;
            ...
            m_phy = device.getIBleDevice().getPhy_private();
        }
        ...
        ReadWriteEvent(BleDevice device, ReadWriteListener.Status status, int gattStatus, Phy phyOptions, double totalTime, double transitTime, boolean solicited)
        {
            this.m_device = device;
            this.m_charUuid = NON_APPLICABLE_UUID;
            this.m_descUuid = NON_APPLICABLE_UUID;
        ...
        /**
         * Forwards {@link BleDevice#getNativeBleService(UUID)}, which will be nonnull
        ...
        /**
         * Forwards {@link BleDevice#getNativeBleCharacteristic(UUID, UUID)}, which will be nonnull
         * if {@link #target()} is {@link ReadWriteListener.Target#CHARACTERISTIC} or {@link ReadWriteListener.Target#DESCRIPTOR}.
         */
        public final @Nullable(Nullable.Prevalence.NORMAL) BleCharacteristic characteristic()
        {
            return device().getNativeCharacteristic(serviceUuid(), charUuid(), descriptorFilter());
        }

        /**
         * Forwards {@link BleDevice#getNativeBleDescriptor_inChar(UUID, UUID)}, which will be nonnull
                ...
                if (target() == Target.RSSI)
                {
                    return Utils_String.toString
                            (
                                    ...
                                    "target", target(),
                                    "rssi", rssi(),
                                    "gattStatus", P_Bridge_Internal.gattStatus(device().getManager().getIBleManager(), gattStatus())
                            );
                }
                ...
                else
                {
                    return Utils_String.toString
                            (
                                    ...
                                    "type", type(),
                                    "charUuid", P_Bridge_Internal.uuidName(device().getManager().getIBleManager(), charUuid()),
                                    "gattStatus", P_Bridge_Internal.gattStatus(device().getManager().getIBleManager(), gattStatus())
                            );
                }
            }
