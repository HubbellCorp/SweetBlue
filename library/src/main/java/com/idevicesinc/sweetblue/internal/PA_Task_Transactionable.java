...
abstract class PA_Task_Transactionable extends PA_Task_RequiresConnection
{
	...
	PA_Task_Transactionable(IBleDevice device, IBleTransaction txn_nullable, boolean requiresBonding, PE_TaskPriority priority)
	{
		super(device, null);
...
//			}
	...
	@Override protected boolean isArmable()
	{
		...
		{
			// Depending on the config option, we may only allow tasks that are part of the current transaction to arm
			IBleTransaction currTrans = getDevice().getTxnManager().getCurrent();
			if (currTrans != null && currTrans.getAtomicity() == BleTransaction.Atomicity.DEVICE_ATOMIC && currTrans != getTxn())
				return false;
