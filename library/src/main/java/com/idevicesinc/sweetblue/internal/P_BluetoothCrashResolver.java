...
final class P_BluetoothCrashResolver {
    ...
    public void notifyScannedDevice(IBluetoothDevice device, BluetoothAdapter.LeScanCallback scanner, ScanCallback callback) {
...
//            Log.w(TAG, "Can't write macs to "+DISTINCT_BLUETOOTH_ADDRESSES_FILE);
    ...
    private final class DiscoveryCancelThread extends Thread
    {
        ...
        DiscoveryCancelThread()
        {
            super(() -> {
                try {
                    ...
                    if (adapter.isDiscovering()) {
                        if (debugEnabled) Log.d(TAG, "Cancelling discovery");
                        adapter.cancelDiscovery();
                    }
                    else {
                        if (debugEnabled) Log.d(TAG, "Discovery not running.  Won't cancel it");
                    }
                } catch (InterruptedException e) {
                    if (debugEnabled) Log.d(TAG, "DiscoveryCanceller sleep interrupted.");
                }
