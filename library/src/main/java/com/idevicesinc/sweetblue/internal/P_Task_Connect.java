...
final class P_Task_Connect extends PA_Task_RequiresBleOn
{
	...
	public P_Task_Connect(IBleDevice device, I_StateListener listener)
	{
		this(device, listener, true, null);
	}
	
	public P_Task_Connect(IBleDevice device, I_StateListener listener, boolean explicit, PE_TaskPriority priority)
	{
	...
	@Override public void execute()
	{
		...
		if( getDevice().getNativeManager().isNativelyConnected() )
		{
		...
		if( getDevice().getNativeManager()./*already*/isNativelyConnecting() )
		{
			// nothing to do
		...
		getManager().ASSERT(!getDevice().getNativeManager().isNativelyDisconnecting(), "");
		...
		if( m_explicit )
		{
			...
			getDevice().nativeManager().connect(useAutoConnect);
