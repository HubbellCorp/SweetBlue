 ...
 *
 * @deprecated Use {@link BleSetupHelper} instead. This class will be removed in v3.1
 */
@Deprecated
public final class BluetoothEnabler
{
    ...
    private void updateBleScanState()
    {
        try
        {
            IBleManager mgr = P_Bridge_User.getIBleManager(bleMngr());
            Method scanstate = mgr.getClass().getDeclaredMethod("setBleScanReady", (Class[]) null);
            scanstate.setAccessible(true);
            scanstate.invoke(mgr, (Object[]) null);
        }
        catch (Exception e)
        {
            e.printStackTrace();
	...
	private void handlePleaseResponse_STEP3a_maybeShowClosingDialog(final BluetoothEnablerFilter.Please please)
	{
		if( please.shouldPopDialog(getStage()) )
		{
			...
			builder.setNeutralButton(P_StringHandler.getString(s_instance.m_defaultActivity.get(), P_StringHandler.OK), (dialog, which) -> {
            });
    ...
    private void handlePleaseResponse_STEP5_maybeShowAppDialog(final BluetoothEnablerFilter.Please please)
    {
        if( please.shouldPopDialog(getStage()) )
        {
           ...
           final AlertDialog.Builder builder = new AlertDialog.Builder(please.activityOrDefault(m_defaultActivity.get()));
			...
			builder.setNegativeButton(P_StringHandler.getString(s_instance.m_defaultActivity.get(), P_StringHandler.DENY), (dialog, which) -> handlePleaseResponse_STEP6_maybeEarlyOutFromDialogResponse(please, /*wasCancelledByDialog=*/true));

			builder.setPositiveButton(P_StringHandler.getString(s_instance.m_defaultActivity.get(), P_StringHandler.ACCEPT), (dialog, which) -> handlePleaseResponse_STEP6_maybeEarlyOutFromDialogResponse(please, /*wasCancelledByDialog=*/false));
