...
public class TransactionTestQueueAtomic extends BaseBleUnitTest
{
    @Test//(timeout = 40000)
    ...
    private void doTest()
    {
        ...
        m_config.defaultTransactionAtomicity = BleTransaction.Atomicity.QUEUE_ATOMIC;
        m_manager.setConfig(m_config);
            ...
            protected void start()
            {
                ...
                final BleRead read = new BleRead(mAuthServiceUuid, mAuthCharUuid).setReadWriteListener(e -> {
                    ...
                    final BleRead read2 = new BleRead(mAuthServiceUuid, mAuthCharUuid).setReadWriteListener(e2 -> {
                        ...
                        succeed();  // Succeedes transaction, NOT test!
                    });
    ...
    private void addTestReadsToQueue(final BleDevice device, int numTasks)
    {
        ...
        for (int i = 0; i < numTasks; ++i)
        {
            ...
            final BleRead read = new BleRead(mInitServiceUuid, mInitCharUuid).setReadWriteListener(e -> {
                ...
                {
                    ++mInitACount;
                    TransactionTestQueueAtomic.this.assertTrue(mTransactionSuccess);

                    if (mInitACount == 2)
                        TransactionTestQueueAtomic.this.assertTrue(mInitBCount == 0);
                    if (mInitACount == 4)
                        TransactionTestQueueAtomic.this.assertTrue(mInitBCount == 2);
                }
                else
                {
                    ++mInitBCount;
                    ...
                    if (mInitBCount == 2)
                        TransactionTestQueueAtomic.this.assertTrue(mInitACount == 2);
                    if (mInitBCount == 3)
                        TransactionTestQueueAtomic.this.assertTrue(mInitACount == 4);
                }

                if (mInitBCount == 4)
                {
                    TransactionTestQueueAtomic.this.assertTrue(mInitACount == 4);  // Make sure the B transactions ran
                    TransactionTestQueueAtomic.this.assertTrue(mTransactionSuccess);  // Make sure the transaction actually ran to completion
                    TransactionTestQueueAtomic.this.succeed();
                }
            });
