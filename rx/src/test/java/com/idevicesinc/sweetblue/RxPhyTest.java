...
public final class RxPhyTest extends RxBaseBleUnitTest
{
    ...
    private void doDefaultTest(Phy phyOption)
    {
        m_config.phyOptions = phyOption;

        m_manager.setConfig(m_config);

        final RxBleDevice device = m_manager.newDevice(Util_Unit.randomMacAddress(), "Speedster");

        m_disposables.add(device.connect().subscribe(() ->
        {
            assertTrue("Expecting " + phyOption.name() + ", but got " + device.getBleDevice().getIBleDevice().getPhy_private().name(), device.getBleDevice().getIBleDevice().getPhy_private() == phyOption);
            succeed();
        }, throwable -> { throw new RuntimeException("Connect failed! This shouldn't happen!!!"); }));
    }
    ...
    private void doManualTest(final Phy phyOption)
    {
        ...
        m_disposables.add(device.connect().subscribe(() ->
        {
            m_disposables.add(device.setPhyOptions(phyOption).subscribe(e1 ->
            {
                assertTrue("Expecting " + phyOption.name() + ", but got " + device.getBleDevice().getIBleDevice().getPhy_private().name(), device.getBleDevice().getIBleDevice().getPhy_private() == phyOption);
                succeed();
            }));
