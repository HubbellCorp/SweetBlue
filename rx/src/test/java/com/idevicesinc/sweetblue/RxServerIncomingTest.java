...
public final class RxServerIncomingTest extends RxBaseBleUnitTest
{
    ...
    public void readServerTest() throws Exception
    {
        ...
        final RxBleServer server = m_manager.getServer(e ->
        {
        ...
        }, db, e -> e.server().connect(macAddress, e1 ->
        {
            ...
            Util_Native.readFromServer(e.server(), macAddress, Uuids.BATTERY_LEVEL, Interval.millis(150));
        }));

        m_disposables.add(server.observeOutgoingEvents().subscribe(e ->
        {
            if (e.macAddress().equals(macAddress))
            {
                assertTrue(e.wasSuccess());
                assertArrayEquals(e.data_sent(), new byte[] { 0x2, 0x3 });
                succeed();
            }
        }));
    ...
    public void writeServerTest() throws Exception
    {
        ...
        final RxBleServer server = m_manager.getServer(e ->
        {
            ...
            {
                assertArrayEquals(e.data_received(), new byte[]{0x1, 0x2});
            }
            return IncomingListener.Please.respondWithSuccess();
        }, db, e -> e.server().connect(macAddress, e1 ->
        {
            ...
            Util_Native.sendWriteToServer(e.server(), macAddress, Uuids.BATTERY_LEVEL, new byte[]{0x1, 0x2}, Interval.millis(150));
        }));
